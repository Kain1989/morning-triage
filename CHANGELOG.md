...
- Optional mail hygiene: `mark_mail_read.py` marks triaged mail as read (Unread filter + right-click "Mark as read", the only combination that works headless; always resets the filter afterwards). Off unless `MT_MARK_MAIL_READ=1`, supports `--dry-run` and `--keep`. Teams conversations were already marked read as a side effect of being opened; mail never was.
- The first look at a Teams conversation now goes back `MT_FIRST_RUN_DAYS` days (default 3) instead of only to yesterday noon, so a fresh install starts with real context. Subsequent runs still resume from each conversation's own watermark.
