...
def main():
    # Teams gets a longer budget than Outlook: its channel scrape walks every team/channel with
    # its own ~100s internal wall-clock, so the old 150s would kill it mid-write. Outlook stays 150s.
    lines = [pull("outlook_pull.py", "outlook", timeout=150),
             pull("teams_pull.py", "teams", timeout=240)]
