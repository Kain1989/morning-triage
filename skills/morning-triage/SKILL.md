...
Below, `PY` = `"$MT_STATE_DIR/venv/bin/python"` (the venv `setup.sh` created) and `ROOT` = the plugin root (`$CLAUDE_PLUGIN_ROOT`). Its collectors are in `"$ROOT/scripts/"`.
...
Run `"$PY" "$ROOT/scripts/selftest.py"` and read its JSON. Exit code `4`/`3` → tell the user to run `setup.sh`. Exit code `2` → note which login is missing (`o365_login.py` and/or `zoom_web_login.py`) and continue with whatever IS signed in. Surface any `warnings` (e.g. `MT_MY_NAME_TOKENS` unset).

## STEP 1 — Teams + Outlook (headless O365)

Run `"$PY" "$ROOT/scripts/pull_inbox.py"`. It writes `$MT_STATE_DIR/inbox/teams_<date>.json` and `outlook_<date>.json` (graceful: an error there is written as JSON, it never blocks). Read both files.
...
Write the digest in `MT_DIGEST_LANG` to `$MT_LOG_DIR/<YYYY-MM-DD>.md` (append a `# Morning triage — <date>` section if the file exists), containing:
